...
DefProp("".base, "includes", { call: String_Includes })
...
String_IndexOf(this, searchString) {
    loop this.Length - searchString.Length + 1
        if SubStr(this, A_Index, searchString.Length) = searchString
            return A_Index
    return -1
}

String_Includes(this, searchString, startPos?) {
    if IsSet(startPos) {
        if startPos < 0
            startPos := this.Length + startPos + 1
        if startPos > this.Length
            return false
    } else {
        startPos := 1
    }
    return String_IndexOf(this, searchString) > 0
}

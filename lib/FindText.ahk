...
Class FindTextClass {  ;// Class Begin
  ...
  __Delete() {
    if (this.bits.hBM)
      Try DeleteObject(this.bits.hBM)
  }
...
int __attribute__((__stdcall__)) PicFind(
  ...
  PicInfo(text) {
    ...
    if (mode = 5) {
      if !(v ~= "^[\s\-\w.]+/[\s\-\w.]+/[\s\-\w./,]+$") { ; <FindPic>
        ...
        DeleteObject(hBM2), color := Trim(arr[1], "-")
        For k1, v1 in arr
  ...
  GetBitsFromScreen(&x := 0, &y := 0, &w := 0, &h := 0
    ...
    if (id) {
      Try
        WinGetClientPos &zx, &zy, &zw, &zh, id ; 修改为客户区
      Catch
        id := 0
    }
    if (!id) {
    ...
    oBM := SelectObject(mDC, bits.hBM)
    if (id) {
      if (mode := this.BindWindow(0, 0, 0, 1)) < 2 {
        hDC := GetDCEx(id, 0, 3)
        BitBlt(mDC, x - zx, y - zy, w, h, hDC, x - zx, y - zy, 0xCC0020 | CAPTUREBLT)
        ReleaseDC(id, hDC)
      }
      else {
        hBM2 := this.CreateDIBSection(zw, zh)
        mDC2 := CreateCompatibleDC()
        ...
        PrintWindow(id, mDC2, (mode > 3) * 3)
        BitBlt(mDC, x - zx, y - zy, w, h, mDC2, x - zx, y - zy, 0xCC0020)
  ...
  CopyHBM(hBM1, x1, y1, hBM2, x2, y2, w, h, Clear := 0) {
    if (w < 1 || h < 1 || !hBM1 || !hBM2)
      return
    mDC1 := CreateCompatibleDC()
    oBM1 := SelectObject(mDC1, hBM1)
    mDC2 := CreateCompatibleDC()
    oBM2 := SelectObject(mDC2, hBM2)
    BitBlt(mDC1, x1, y1, w, h, mDC2, x2, y2, 0xCC0020)
    if (Clear)
      BitBlt(mDC1, x1, y1, w, h, mDC1, x1, y1, MERGECOPY := 0xC000CA)
  ...
  CaptureCursor(hDC := 0, zx := 0, zy := 0, zw := 0, zh := 0, get_cursor := 0) {
    ...
    DrawIconEx(hDC, x - xCenter - zx, y - yCenter - zy, hCursor, 0, 0, 0, 0, 3)

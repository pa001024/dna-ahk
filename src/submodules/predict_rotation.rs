use opencv::{
    core::{self, BorderTypes, CV_8UC1, CV_8UC4, CV_32F, Mat, Point2f, Rect, Size, Vector},
    imgproc,
    prelude::*,
};
use std::mem::zeroed;
...
fn create_error(message: &str) -> windows::core::Error {
    Error::new(windows::core::HRESULT(0x80070057u32 as i32), message)
}

pub(crate) fn hbitmap_to_bgr_mat(hbmp: HBITMAP) -> Result<Mat> {
    unsafe {
        ...
        if got == 0 || bmp.bmWidth <= 0 || bmp.bmHeight == 0 {
            return Err(create_error("GetObjectW failed"));
        }
        ...
        if hdc.0 == std::ptr::null_mut() {
            return Err(create_error("CreateCompatibleDC failed"));
        }
        let old: HGDIOBJ = SelectObject(hdc, HGDIOBJ::from(hbmp));
        if old.0 == std::ptr::null_mut() {
            let _ = DeleteDC(hdc);
            return Err(create_error("SelectObject failed"));
        }
        ...
        if scanlines == 0 {
            return Err(create_error("GetDIBits failed"));
        }

        // Build OpenCV BGRA Mat (top-down)
        let mut mat_bgra = Mat::new_rows_cols(height, width, CV_8UC4)
            .map_err(|e| create_error(&format!("Mat::new_rows_cols: {e}")))?;
        ...
        imgproc::cvt_color(&mat_bgra, &mut mat_bgr, imgproc::COLOR_BGRA2BGR, 0)
            .map_err(|e| create_error(&format!("cvtColor BGRA2BGR: {e}")))?;
        Ok(mat_bgr)
    }
}

// Identity "convolution" matching the Python integer // behavior for the kernels used.
fn convolve_identity(arr: &[i64], _kernel: i32) -> Vec<i64> {
    arr.to_vec()
}
...
pub(crate) fn predict_rotation(img_bgr: &Mat) -> Result<i32> {
    let d = img_bgr.rows();
    if d <= 0 {
        return Err(create_error("图像尺寸无效"));
    }
    let scale = 1i32;
    ...
    imgproc::cvt_color(img_bgr, &mut yuv, imgproc::COLOR_BGR2YUV, 0)
        .map_err(|e| create_error(&format!("cvt_color: {e}")))?;
    let mut channels = Vector::<Mat>::new();
    core::split(&yuv, &mut channels).map_err(|e| create_error(&format!("split: {e}")))?;
    if channels.len() != 3 {
        return Err(create_error("Unexpected YUV channel count"));
    }
    let v = channels
        .get(2)
        .map_err(|e| create_error(&format!("get channel 2: {e}")))?;

    // img2 = 128 - v
    let mut img2 = Mat::default();
    let zero = Mat::zeros(v.rows(), v.cols(), CV_8UC1)
        .map_err(|e| create_error(&format!("Mat::zeros: {e}")))?
        .to_mat()
        .map_err(|e| create_error(&format!("to_mat: {e}")))?;
    core::add_weighted(&v, -1.0, &zero, 0.0, 128.0, &mut img2, -1)
        .map_err(|e| create_error(&format!("add_weighted: {e}")))?;
    ...
    .map_err(|e| create_error(&format!("gaussian_blur: {e}")))?;
    ...
    .map_err(|e| create_error(&format!("warp_polar: {e}")))?;

    // transpose
    let mut polar_t = Mat::default();
    core::transpose(&polar, &mut polar_t).map_err(|e| create_error(&format!("transpose: {e}")))?;

    // crop rows [d*2/10 .. d*5/10)
    let row_start = (d * 2) / 10;
    let row_end = (d * 5) / 10;
    let remap2 = Mat::roi(
        &polar_t,
        Rect::new(0, row_start, polar_t.cols(), row_end - row_start),
    )
    .map_err(|e| create_error(&format!("ROI: {e}")))?;
    ...
    .map_err(|e| create_error(&format!("Scharr: {e}")))?;
    ...
    let gradx_cont = if gradx.is_continuous() {
        ...
        gradx
            .copy_to(&mut tmp)
            .map_err(|e| create_error(&format!("copyTo: {e}")))?;
        tmp
    };
    ...
    for offset in (-kernel2 + 1)..kernel2 {
        let shift1 = -(d * scale) / 4 + offset;
        let r_shift1 = roll_vec(&r2, shift1 as isize);
        let r_conv1 = convolve_identity(&r_shift1, 3 * scale);
        ...
        let r_conv2 = convolve_identity(&r_shift2, 10 * scale);
        for i in 0..period {
            result[i] -= (l2[i] * r_conv2[i]) / 5;
        }

        let result_final = convolve_identity(&result, 3 * scale);
    ...
    // degree = idx / (d*scale) * 360 + 135
    let deg = ((argm as f64) / ((d * scale) as f64) * 360.0 + 135.0) % 360.0;
    Ok(deg as i32)
}
            ...
            Ok(mat_bgr) => {
                ...
                match predict_rotation(&mat_bgr) {
                    Ok(predicted_angle) => {
                        ...
                        // 检查预测结果是否接近121度（允许±1度误差）
                        let expected_angle = 121;
                        assert!(
                            (predicted_angle as i32 - expected_angle).abs() <= 1,
                            "预测结果与预期不一致: 期望接近121度，实际为{}度",
                            predicted_angle
                        );
                        println!("测试通过: 预测结果{}接近预期的121度", predicted_angle);
                    }
                    Err(e) => {
                        println!("预测失败: {:?}", e);
                        assert!(false, "预测失败: {:?}", e);

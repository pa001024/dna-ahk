...
class DNAGui {
    ...
    static InitGui() {
        ...
        char := oGui.AddDropDownList("x16 y16 w120 Section", ["水母E", "黎瑟Q", "黎瑟EQ循环", "近战蓄力"])
        char.Text := IniRead("dna-ahk.ini", "gui", "char", "水母E")
        char.OnEvent("Change", (*) => IniWrite(char.Text, "dna-ahk.ini", "gui", "char"))
        oGui.char := char
        ...
        mode.Text := IniRead("dna-ahk.ini", "gui", "mode", "30线索")
        mode.OnEvent("Change", (*) => IniWrite(mode.Text, "dna-ahk.ini", "gui", "mode"))
        oGui.mode := mode
        ...
        mute.Value := IniRead("dna-ahk.ini", "gui", "mute", 0)
        mute.OnEvent("Click", (*) => this.Mute())
        oGui.mute := mute
        melee := oGui.AddCheckbox("x+8 yp w120", "穿引共鸣")
        melee.Value := IniRead("dna-ahk.ini", "gui", "melee", 0)
        melee.OnEvent("Click", (*) => IniWrite(melee.Value, "dna-ahk.ini", "gui", "melee"))
        oGui.melee := melee
        ...
        oGui.AddText("x16 y+12 w120 Section", "当前波次")
        wave := oGui.AddText("x+8 yp w120", "-")
        oGui.wave := wave
    ...
    static Mute() {
        ...
        IniWrite(this.oGui.mute.Value, "dna-ahk.ini", "gui", "mute")
